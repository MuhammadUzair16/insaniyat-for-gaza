...
jQuery.extend( {
	...
	// the ready events fires. See trac-6781
	readyWait: 1,
...
function leverageNative( el, type, isSetup ) {
	...
	// Missing `isSetup` indicates a trigger call, which must force setup through jQuery.events.add
	if ( !isSetup ) {
	...
	jQuery.event.add( el, type, {
		...
		handler: function( event ) {
			...
			if ( ( event.isTrigger & 1 ) && this[ type ] ) {
				...
				// the native events and prevent that from happening again here.
			...
			// Fire an inner synthetic events with the original arguments
				...
				// Abort handling of the native events by all jQuery handlers while allowing
				// native handlers on the same element to run. On target, this is achieved
				// by stopping immediate propagation just on the jQuery events. However,
				// the native events is re-wrapped by a jQuery one on each level of the
...
jQuery.each( { focus: "focusin", blur: "focusout" }, function( type, delegateType ) {
	...
	function focusMappedHandler( nativeEvent ) {
		if ( document.documentMode ) {
			...
			// `handle` from private data would already wrap the events, but we need
			...
			if ( event.target === event.currentTarget ) {
				...
				// `jQuery.events.add`, so events handle will already have been set
				// by this point.
				handle( event );
			}
		} else {
	...
	jQuery.event.special[ type ] = {
		...
		// a leveraged native-events stack
		_default: function( event ) {
	...
	//
	// Support: IE 9 - 11+
	// To preserve relative focusin/focus & focusout/blur events order guaranteed on the 3.x branch,
...
jQuery.extend( jQuery.event, {
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		// Determine events propagation path in advance, per W3C events spec (trac-9951)
...
jQuery.extend( {
	...
	ajax: function( url, options ) {
		...
		// Don't fire events if jQuery.events is undefined in an AMD-usage scenario (trac-15118)
